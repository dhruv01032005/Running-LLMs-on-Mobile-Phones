...
class ChatViewModel(application: Application) : AndroidViewModel(application), LlmCallback {
    // Chaquopy Python execution support
    init {
        try {
            // Initialize Chaquopy if not already initialized
            com.chaquo.python.Python.start(AndroidPlatform(application))
            System.out.println("chaquo loaded")
    ...
    override fun onResult(result: String) {
        ...
        if (processedResult == PromptFormat.getStopToken(currentSettingsFields.modelType)) {
            // Check if the accumulated resultMessage text is Python code
            var codeText = resultMessage?.text ?: ""
            codeText = codeText.trimIndent()
            val py = Python.getInstance()
            val op = py.getModule("test").callAttr("main", codeText)
            System.out.println("Code ran: " + op)
            val outputText = "\nExecuted output: " + op
            resultMessage?.appendText(outputText)
//            if (isPythonCode(codeText)) {
//                val pythonOutput = runCustomPythonCode(codeText)
//                val outputText = if (pythonOutput.isNullOrBlank()) {
//                    "\n[Python code executed, but no output was produced.]"
//                } else {
//                    "\n[Python output:]\n$pythonOutput"
//                }
...
//            }
        ...
        if (keepResult) {
            ...
            // If the LLM response is Python code, execute it and append the output or error inline
            
        }

    }

    // Helper to detect Python code (simple heuristic: starts with 'def', 'import', or contains 'print(')
    private fun isPythonCode(text: String): Boolean {
        val trimmed = text.trim()
        return trimmed.contains("def ") || trimmed.contains("import ") || trimmed.contains("return ")
    }

    // Run Python code using Chaquopy and return output or error
    ...
    //         outputStream.callAttr("getvalue").toString()
    //     } catch (e: Exception) {
    ...
    fun runCustomPythonCode(processedResult: String): String {
    return try {
        val py = com.chaquo.python.Python.getInstance()
        val sys = py.getModule("sys")
        val io = py.getModule("io")
        ...
        // Copy .pkl files from assets to app files dir if not already present
        val context = getApplication<Application>().applicationContext
        val assetNames = listOf("python/main_data.pkl", "python/ncap_funding_data.pkl", "python/states_data.pkl")
        val filesDir = context.filesDir
        for (assetName in assetNames) {
            val destFile = java.io.File(filesDir, assetName)
            if (!destFile.exists()) {
...
//         val pythonCode = """
// import pandas as pd
// main_data = pd.read_pickle('main_data.pkl')
// ncap_data = pd.read_pickle('ncap_funding_data.pkl')
// states_data = pd.read_pickle('states_data.pkl')
// def exec_code(code):
//     local_vars={}
//     try:
//         exec(code, globals(), local_vars)
//     except Exception as e:
//         return None, '', f'Error while executing code string: {e}'
//     get_response = local_vars['get_response']
//     try:
//         result = get_response(main_data, states_data, ncap_data)
//         return(result)
//     except Exception as e:
//         return None
// generated_code = '''${processedResult}'''
// generated_code = generated_code.replace('<code>', '')
// generated_code = generated_code.replace('</code>', '')
// output = exec_code(generated_code)
// print(output)
// """

        py.getModule("builtins").callAttr("exec", pythonCode)
        outputStream.callAttr("getvalue").toString()
    } catch (e: Exception) {
        "Python error: ${e.message}"
    }
