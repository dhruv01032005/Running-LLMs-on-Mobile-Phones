...
## Project Guide (This App)

This section explains how this app is built, what it does, how to add models on your phone, and how to install the app on Android.
...
- ExecuTorch Android runtime (`org.pytorch:executorch-android`)
- JNI/native backends for on-device model execution
- Chaquopy (Python on Android) for Python-based post-processing workflows
- SharedPreferences-based settings/state for model configs and chat behavior

High-level flow:

1. `WelcomeActivity` is the launcher screen.
2. `ModelSettingsActivity` lets you choose model files, tokenizer, backend, prompt format, and optional LoRA mode.
3. `MainActivity` hosts `ChatScreen`, where prompts/media are sent to `ChatViewModel`.
4. `ChatViewModel` loads the selected model(s), runs generation, and streams responses into chat.

### 2) Main Functions of the App

- On-device inference (no server required) with ExecuTorch.
- Backend selection for supported delegates (XNNPACK / Qualcomm / MediaTek / Vulkan based on build/device support).
- Single-model mode and LoRA multi-model mode.
- Text chat plus multimodal support for supported model types:
  - image input (gallery/camera) for vision-capable models
  - audio feature input (`.bin`) for audio-capable models such as Voxtral setup
- Prompt controls (system prompt, user prompt templates, think mode).
- Built-in model preset screen for downloading and selecting predefined model configs.
- Runtime logs, memory usage indicator, and chat persistence controls.

### 3) How To Add Model Files on Your Mobile for Inference

The model settings screen scans these device folders:

- Model files (`.pte`): `/data/local/tmp/llama/`
- Tokenizer files (`.bin`, `.json`, `.model`): `/data/local/tmp/llama/`
...
adb push <your_tokenizer>.model /data/local/tmp/llama/
```

Then in the app:

1. Open `Load Model` from the welcome screen.
2. Select model/tokenizer/data path in settings.
3. Tap `Load Model`.
4. Return to chat and start inference.
...
Install debug build directly from source:
...
After installation, open the app on phone, add/select model files, and run inference offline.
